...
bool CDeviceHandler::G2Update(unsigned char* file_buf)
{
    ...
    if(nBootUpdate_finish <= 0)
    {
        ...
        SaveHistory("TxRequestBootUpdate Error");
        return iRet;
    }
    ...
    if(nCUUpdate_finish <= 0)
    {
        ...
        SaveHistory("TxRequestCuUpdate Error");
        return iRet;
    }
    ...
    if(nFWUpdate_finish <= 0)
    {
        ...
        SaveHistory("TxRequestFWUpdate Error");
        return iRet;
    }
    ...
    if(m_sFwVersion == "")
    {
        ...
        SaveHistory("Updated FW Version Get Fail");
    }
    else
    {
        if(m_bVerHex)
        {
            ...
            SaveHistory("(HEX)Updated FW Ver : "+m_sFwVersion);
        }
        else
        {
            LOG_G2(CLog::getLogOwner(), TAG, "Updated FW Ver : %s",m_sFwVersion.c_str());
            SaveHistory("Updated FW Ver : "+m_sFwVersion);
        }
    }

    if((nBootUpdate_finish == 1)  && (nCUUpdate_finish == 1) && (nFWUpdate_finish == 1))
    {
        SaveHistory("FINISH : UPDATE SUCCESS");
        return true;
    }

    SaveHistory("FINISH : UPDATE FAIL");
    return false;
}

/////////////////////////////////////////////////////////////////////////
// history log
void CDeviceHandler::SaveHistory(string msg)
{
    string datetime = GetDateTimeString();
    string historyfname = log_path + "/debugmsg.txt";
    fstream fs;
...
bool CDeviceHandler::CheckAndCreate2(string folder)
{
    struct stat sb;
    bool bFound = false;
